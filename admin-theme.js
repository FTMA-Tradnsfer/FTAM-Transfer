(function(){
  ...
  function mount(){
    ...
    const s=document.createElement('section');s.id='ftmaThemePanel';s.className='admin-card admin-wide ftma-theme-panel';s.innerHTML=`<p class="eyebrow">SITE THEME CONTROL</p><h2>이적시장 테마</h2><p>관리자만 전체 사이트의 활성 테마를 변경할 수 있습니다. 기존 기본 테마는 삭제되지 않습니다.</p><div class="ftma-theme-buttons"><button type="button" data-theme-button="default">기본 테마</button><button type="button" data-theme-button="summer">☀️ 여름 이적시장</button><button type="button" data-theme-button="winter">🎄 크리스마스 이적시장</button></div><div class="ftma-theme-status" id="ftmaThemeStatus"></div><div style="margin-top:16px;padding-top:16px;border-top:1px solid #2b2d31"><button type="button" id="ftmaTransferTestButton" class="primary-btn">🧪 이적 시스템 테스트 열기</button><p style="margin:8px 0 0;color:#77736c;font-size:9px">실제 이적·선수·구단 자금에는 영향을 주지 않는 테스트 환경입니다.</p></div>`;
    grid.appendChild(s);
    const testBtn=s.querySelector('#ftmaTransferTestButton');
    testBtn.onclick=()=>{window.open('transfer-test.html','_blank','noopener,noreferrer')};
    s.querySelectorAll('[data-theme-button]').forEach(b=>b.onclick=async()=>{const theme=b.dataset.themeButton;const status=document.getElementById('ftmaThemeStatus');s.querySelectorAll('[data-theme-button]').forEach(x=>x.disabled=true);status.textContent='저장 중...';try{await set(theme);window.FTMATheme?.apply(theme);s.querySelectorAll('[data-theme-button]').forEach(x=>x.classList.toggle('active',x.dataset.themeButton===theme));status.textContent=`현재 활성 테마: ${labels[theme]}`;}catch(e){status.textContent='저장 실패: '+e.message}finally{s.querySelectorAll('[data-theme-button]').forEach(x=>x.disabled=false)}});
    get().then(theme=>{window.FTMATheme?.apply(theme);s.querySelectorAll('[data-theme-button]').forEach(x=>x.classList.toggle('active',x.dataset.themeButton===theme));document.getElementById('ftmaThemeStatus').textContent=`현재 활성 테마: ${labels[theme]||labels.default}`}).catch(e=>{document.getElementById('ftmaThemeStatus').textContent=e.message});
  }
