(function(){
  ...
  function mount(){
    ...
    const s=document.createElement('section');s.id='ftmaThemePanel';s.className='admin-card admin-wide ftma-theme-panel';s.innerHTML=`<p class="eyebrow">SITE THEME CONTROL</p><h2>이적시장 테마</h2><p>관리자만 전체 사이트의 활성 테마를 변경할 수 있습니다. 기존 기본 테마는 삭제되지 않습니다.</p><div class="ftma-theme-buttons"><button type="button" data-theme-button="default">기본 테마</button><button type="button" data-theme-button="summer">☀️ 여름 이적시장</button><button type="button" data-theme-button="winter">❄️ 겨울 이적시장</button></div><div class="ftma-theme-status" id="ftmaThemeStatus"></div>`;
    grid.appendChild(s);
