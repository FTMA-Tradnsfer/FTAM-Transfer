/* FTMA market/player search — no observers, no DB calls, mobile/PC safe. */
(function(){
'use strict';
const norm=v=>String(v??'').normalize('NFKC').replace(/\s+/g,'').toLocaleLowerCase('ko-KR');
function marketRows(){return Array.from(document.querySelectorAll('#marketList .market-directory-row'));}
function filterMarket(){
 const input=document.getElementById('ftmaMarketUnifiedInput');
 if(!input)return;
 const rows=marketRows(),q=norm(input.value);
 let visible=0;
 rows.forEach(row=>{
  const name=row.querySelector('.market-player-name-line b,.market-player-main b')?.textContent||'';
  const ok=!q||norm(name).includes(q);
  row.classList.toggle('ftma-search-hidden',!ok);
  if(ok)visible++;
 });
 const empty=document.getElementById('ftmaMarketSearchEmpty');
 if(empty)empty.remove();
 if(q&&rows.length&&!visible){
  const e=document.createElement('div');e.id='ftmaMarketSearchEmpty';e.className='ftma-search-empty';e.textContent='검색 결과가 없습니다.';
  document.getElementById('marketList')?.appendChild(e);
 }
 const count=document.getElementById('ftmaMarketUnifiedCount');
 if(count)count.textContent=q?`${visible}명 검색됨 / ${rows.length}명`:`${rows.length}명 등록`;
}
function renderNegotiationResults(){
 const select=document.getElementById('realPlayerPick'),input=document.getElementById('ftmaNegotiationPlayerInput');
 if(!select||!input)return;
 const q=norm(input.value),options=Array.from(select.options).filter(o=>o.value);
 const matches=options.filter(o=>!q||norm(o.textContent).includes(q)).slice(0,50);
 let results=document.getElementById('ftmaNegotiationPlayerResults');
 if(!results){results=document.createElement('div');results.id='ftmaNegotiationPlayerResults';results.className='ftma-neg-results';input.parentElement?.insertAdjacentElement('afterend',results)}
 results.textContent='';
 matches.forEach(o=>{const b=document.createElement('button');b.type='button';b.dataset.playerValue=o.value;b.textContent=o.textContent;results.appendChild(b)});
 const c=document.getElementById('ftmaNegotiationPlayerCount');if(c)c.textContent=q?`${matches.length}명 검색됨`:`${options.length}명`;
}
function install(){
 if(document.getElementById('ftmaUnifiedSearchStyles'))return;
 const style=document.createElement('style');style.id='ftmaUnifiedSearchStyles';style.textContent=`
.ftma-unified-search{display:flex;align-items:center;gap:10px;margin:10px 0 12px;padding:10px;border:1px solid #303338;background:#111315}
.ftma-unified-search input{flex:1;min-width:0;width:100%;box-sizing:border-box;background:#090b0d;border:1px solid #3a3d42;color:#eee;padding:11px 12px;outline:0;font-size:12px}
.ftma-unified-search input:focus{border-color:#caa968}
.ftma-unified-search button{appearance:none;border:1px solid #665535;background:transparent;color:#e8d3a0;padding:10px 16px;cursor:pointer;font-weight:800}
.ftma-search-hidden{display:none!important}
.ftma-search-empty{padding:16px;text-align:center;color:#77736b;font-size:10px}
@media(max-width:700px){.ftma-unified-search{display:grid;grid-template-columns:1fr auto;gap:7px}.ftma-unified-search input{min-height:42px;font-size:14px}.ftma-unified-search button{min-height:42px;padding:8px 14px}}
`;
 document.head.appendChild(style);
 let panel=document.getElementById('ftmaMarketUnifiedSearch');
 const list=document.getElementById('marketList');
 if(list&&!panel){
  panel=document.createElement('div');panel.id='ftmaMarketUnifiedSearch';panel.className='ftma-unified-search';
  panel.innerHTML='<input id="ftmaMarketUnifiedInput" type="search" inputmode="search" autocomplete="off" placeholder="선수 이름으로 검색"><button id="ftmaMarketSearchButton" type="button">검색</button>';
  list.parentElement?.insertBefore(panel,list);
 }
 const input=document.getElementById('ftmaMarketUnifiedInput');
 const button=document.getElementById('ftmaMarketSearchButton');
 if(input&&!input.dataset.ftmaBound){input.dataset.ftmaBound='1';input.addEventListener('input',filterMarket);input.addEventListener('compositionend',filterMarket);input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();filterMarket();}})}
 if(button&&!button.dataset.ftmaBound){button.dataset.ftmaBound='1';button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();filterMarket();});}
 const nInput=document.getElementById('ftmaNegotiationPlayerInput');
 if(nInput&&!nInput.dataset.ftmaBound){nInput.dataset.ftmaBound='1';nInput.addEventListener('input',renderNegotiationResults);nInput.addEventListener('compositionend',renderNegotiationResults);nInput.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();renderNegotiationResults();}})}
 filterMarket();renderNegotiationResults();
}
function start(){install();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();
