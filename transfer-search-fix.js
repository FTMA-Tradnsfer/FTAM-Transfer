/* FTMA unified player-name search — market + negotiation, async-safe and mobile-safe. */
(function(){
'use strict';
...
.ftma-neg-results b{font-size:12px}
.ftma-neg-results small{color:#8f8b84;font-size:9px}
.ftma-neg-results .empty{padding:11px;text-align:center;color:#77736b;border:1px dashed #2b2e33}
...
function installMarket(){
 ...
 if(!panel){
  ...
  panel.innerHTML='<label>PLAYER SEARCH</label><input id="ftmaMarketUnifiedInput" type="search" inputmode="search" autocomplete="off" placeholder="선수 이름으로 검색"><small id="ftmaMarketUnifiedCount"></small>';
  const h=list.closest('.directory-section')?.querySelector('.directory-heading');
  if(h)h.insertAdjacentElement('afterend',panel);else list.parentElement?.insertBefore(panel,list);
 }
 const input=document.getElementById('ftmaMarketUnifiedInput');
 if(input&&!input.dataset.ftmaBound){
  input.dataset.ftmaBound='1';
  ['input','search','change'].forEach(e=>input.addEventListener(e,filterMarket));
  input.addEventListener('compositionend',filterMarket);
 }
 filterMarket();
}

function filterMarket(){
 const list=document.getElementById('marketList'),input=document.getElementById('ftmaMarketUnifiedInput');
 if(!list||!input)return;
 const q=norm(input.value);
 const rows=[...list.querySelectorAll('.market-directory-row')];
 let visible=0;
 rows.forEach(row=>{
  const name=row.querySelector('.market-player-name-line b')?.textContent||'';
 ...
 if(q&&rows.length&&!visible){
  if(!empty){empty=document.createElement('div');empty.className='ftma-search-empty';empty.textContent='검색 결과가 없습니다.';list.appendChild(empty)}
  empty.style.display='block';
 }else if(empty)empty.style.display='none';
 const c=document.getElementById('ftmaMarketUnifiedCount');
 if(c)c.textContent=q?`${visible}명 검색됨 / ${rows.length}명`:`${rows.length}명 등록`;
}
...
function installNegotiation(){
 ...
 if(!panel){
  ...
  panel.className='ftma-unified-search';
  panel.innerHTML='<label>PLAYER SEARCH</label><input id="ftmaNegotiationPlayerInput" type="search" inputmode="search" autocomplete="off" placeholder="선수 이름으로 검색"><small id="ftmaNegotiationPlayerCount"></small>';
 ...
 if(!input||input.dataset.ftmaBound)return;
 input.dataset.ftmaBound='1';
 ['input','search','change'].forEach(e=>input.addEventListener(e,renderNegotiationResults));
...
function renderNegotiationResults(){
 ...
 const matches=options.filter(o=>!q||norm(o.textContent).includes(q)).slice(0,30);
 let results=document.getElementById('ftmaNegotiationPlayerResults');
 if(!results){
  results=document.createElement('div');
  results.id='ftmaNegotiationPlayerResults';
  results.className='ftma-neg-results';
  input.parentElement?.insertAdjacentElement('afterend',results);
 }
 results.innerHTML=matches.length?matches.map(o=>{
  const parts=o.textContent.split(' · ');
  return `<button type="button" data-player-value="${String(o.value).replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" data-player-name="${String(parts[0]||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"><b>${parts[0]||''}</b><small>${parts.slice(1).join(' · ')}</small></button>`;
 }).join(''):'<div class="empty">검색 결과가 없습니다.</div>';
 results.onclick=e=>{
  const btn=e.target.closest('button[data-player-value]');
  if(!btn)return;
  select.value=btn.dataset.playerValue||'';
  input.value=btn.dataset.playerName||'';
  select.dispatchEvent(new Event('change',{bubbles:true}));
  results.innerHTML='';
  const c=document.getElementById('ftmaNegotiationPlayerCount');
  if(c)c.textContent='선택 완료';
 };
 const c=document.getElementById('ftmaNegotiationPlayerCount');
 if(c)c.textContent=q?`${matches.length}명 검색됨`:`${options.length}명`;
}
...
function start(){
 ...
 const list=document.getElementById('marketList');
 if(list){
  const obs=new MutationObserver(()=>requestAnimationFrame(installMarket));
  obs.observe(list,{childList:true});
 }
 const left=document.getElementById('leftContent');
 if(left){
  const obs2=new MutationObserver(()=>requestAnimationFrame(installNegotiation));
  obs2.observe(left,{childList:true});
 }
