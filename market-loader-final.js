/* FTMA stable transfer-market loader: never block the player list on market-state RPC. */
(function(){
  ...
  const timeout=p=>Promise.race([p,new Promise(r=>setTimeout(()=>r(null),4000))]);
  async function load(){
    ...
    try{
      const r=await timeout(db.from('players').select('id,name,photo_url,nationality,position,market_value,current_club_id,age,birth_date,shirt_number,is_loan,is_nfs').order('market_value',{ascending:false}));
      if(!r||r.error)throw(r?.error||new Error('players timeout'));
      const players=r.data||[];
      const cr=await timeout(db.from('clubs').select('id,name,logo_url'));
      const clubs=cr?.data||[];
      ...
      box.innerHTML=market.length?market.map((p,i)=>`<div class="market-directory-row"><a class="market-player-cell" href="player.html?id=${encodeURIComponent(p.id)}">${p.photo_url?`<img class="market-player-photo" src="${esc(p.photo_url)}" alt="${esc(p.name)}" loading="lazy" decoding="async">`:''}<span class="player-rank">${String(i+1).padStart(2,'0')}</span><div class="market-player-main"><div class="market-player-name-line"><b>${esc(p.name)}</b></div><small>${esc(p.position||'미정')} · ${esc(p.nationality||'국적 미상')} · ${esc(p.club?.name||'미소속')} · ${age(p)}세 · ${p.shirt_number!=null?`#${p.shirt_number}`:'번호 미등록'}</small></div></a><strong>${money(p.market_value)}</strong>${loggedIn()?'<button type="button" class="market-offer-btn" data-player-id="'+esc(p.id)+'">이적 제안</button>':'<em class="status open">시장 등록</em>'}</div>`).join(''):'<div class="loading">현재 이적 가능한 선수가 없습니다.</div>';
      loaded=true;
      ...
      console.error('[FTMA] stable market loader failed',e);
