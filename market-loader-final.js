...
const MARKET_CACHE_KEY='ftma_market_players_v3',MARKET_CACHE_TTL=120000;
async function loadPlayers(){
  ...
  const r=await timeout(fetch('/api/public-data?resource=market',{cache:'default'}));
  if(!r.ok)throw new Error(`HTTP ${r.status}`);
  const rows=await r.json();
  if(!Array.isArray(rows))throw new Error('시장 데이터 형식 오류');
...
function loadHotfix(){if(document.getElementById('ftmaOfferHotfixScript'))return;const s=document.createElement('script');s.id='ftmaOfferHotfixScript';s.src='transfer-market-offer-hotfix.js?v=20260902opt2';document.head.appendChild(s)}
