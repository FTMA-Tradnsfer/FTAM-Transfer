...
function renderClubs(list,players){
  ...
  box.innerHTML=list.length?list.map(c=>{
    ...
    return `<a class="directory-club" href="club.html?id=${encodeURIComponent(c.id)}"><div class="club-mark">${mark}</div><div class="club-info"><h3>${esc(c.name)}</h3><p>${esc(c.league||'리그 미상')}</p>${c.description?`<small>${esc(c.description)}</small>`:''}</div><strong>${count}<small>명</small></a>`
  }).join(''):'<div class="loading">검색 결과가 없습니다.</div>';
}
