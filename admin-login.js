(function(){
  ...
  const form=document.getElementById('adminLoginForm'),lock=document.getElementById('adminLock'),app=document.getElementById('adminApp'),input=document.getElementById('adminPassword'),button=document.getElementById('adminLoginButton'),error=document.getElementById('adminLoginError');
  ...
  function loadFinancePanel(){
    if(!document.getElementById('financeAdminScript')){const s=document.createElement('script');s.id='financeAdminScript';s.src='finance-admin-section.js?v=20260828financeadmin2';document.body.appendChild(s)}else window.dispatchEvent(new Event('ftma-admin-authenticated'));
  }
  function showApp(){document.body.classList.remove('admin-locked');document.body.dataset.adminAuthenticated='true';app.hidden=false;app.style.display='block';if(lock&&lock.parentNode)lock.remove();setTimeout(loadFinancePanel,0);}
  function restoreSession(){const token=getToken(),expires=getExpires();if(!token)return false;if(expires){const time=Date.parse(expires);if(Number.isFinite(time)&&time<=Date.now()){clearSession();return false}}showApp();return true;}
  ...
  async function requestJson(url,options,timeoutMs){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{const response=await fetch(url,{...options,cache:'no-store',signal:controller.signal});const text=await response.text();let data=null;try{data=text?JSON.parse(text):null}catch(_){throw new Error('서버 응답이 올바른 JSON이 아닙니다.')}if(!response.ok)throw new Error(data?.message||data?.error||text||('HTTP '+response.status));return data}catch(e){if(e?.name==='AbortError')throw new Error('TIMEOUT');throw e}finally{clearTimeout(timer)}}
  ...
  async function login(){if(button.disabled)return;const password=input.value;if(!password){error.textContent='관리자 비밀번호를 입력해주세요.';input.focus();return}busy(true);error.textContent='';try{let data;try{data=await callProxy(password)}catch(proxyError){data=await callDirectRpc(password)}saveSession(data);input.value='';showApp();busy(false);if(typeof window.refreshAdminData==='function')Promise.resolve(window.refreshAdminData()).catch(()=>{});}catch(err){error.textContent='로그인 실패: '+(err?.message==='TIMEOUT'?'로그인 서버 응답 시간이 초과되었습니다. 다시 시도해주세요.':(err?.message||'알 수 없는 오류'));busy(false);input.focus()}}
  document.addEventListener('submit',e=>e.preventDefault(),true);document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key==='NumpadEnter')&&document.activeElement===input){e.preventDefault();e.stopImmediatePropagation();login()}},true);button.type='button';button.addEventListener('click',login);localStorage.removeItem('ftma_admin_token');localStorage.removeItem('ftma_admin_expires');if(!restoreSession()){clearSession();button.disabled=false;button.textContent='관리자 페이지 입장'}window.addEventListener('pageshow',restoreSession);
})();