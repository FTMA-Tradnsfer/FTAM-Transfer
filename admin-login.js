(function(){
  ...
  async function login(e){
    ...
    try{
      const response=await fetch(RPC,{method:'POST',mode:'cors',cache:'no-store',signal:controller.signal,headers:{'apikey':KEY,'Authorization':'Bearer '+KEY,'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({p_password:password})});
      const text=await response.text();
      let data=null;try{data=text?JSON.parse(text):null}catch(_){data=null;}
      if(!response.ok)throw new Error(data?.message||text||('인증 서버 오류 ('+response.status+')'));
      openAdmin(data);
    }catch(err){
