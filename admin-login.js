(function(){
  'use strict';
  // Load the same seasonal theme used by the public site before authentication UI is shown.
  ...
  function restoreSession(){
    ...
    if(expires){const time=Date.parse(expires);if(Number.isFinite(time)&&time<=Date.now()){clearSession();return false;}}
    showApp();return true;
  }
  ...
  async function requestJson(url,options,timeoutMs){
    ...
    try{
      ...
      const text=await response.text();let data=null;try{data=text?JSON.parse(text):null}catch(_){throw new Error('서버 응답이 올바른 JSON이 아닙니다.')}
  ...
  async function login(){
    ...
    try{
      let data;try{data=await callProxy(password)}catch(proxyError){data=await callDirectRpc(password)}
      saveSession(data);input.value='';showApp();busy(false);if(typeof window.refreshAdminData==='function')Promise.resolve(window.refreshAdminData()).catch(()=>{});
