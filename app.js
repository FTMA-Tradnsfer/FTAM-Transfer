...
async function loadData(){
  ...
  if(pe||ce||oe){console.error(pe||ce||oe);return showError('FTMA 데이터베이스 연결에 문제가 있습니다.');}
...
function render(ps,cs,offers){
  ...
  const pending=offers.filter(x=>x.status==='pending');document.getElementById('pendingCount').textContent=pending.length;document.getElementById('offerCount').textContent=`${pending.length}건의 진행 중 제안`;
  ...
  document.getElementById('marketRows').innerHTML=ps.length?ps.map((p,i)=>`<article class="player-row"><div class="player"><span class="avatar">${String(i+1).padStart(2,'0')}</span><span><b>${esc(p.name)}</b><small>${esc(p.nationality||'—')} • ${p.age||'유망주'}</small></span></div><span>${esc(p.position||'—')}</span><span>${esc(p.clubs?.name||'자유계약')}</span><strong>${money(p.market_value)}</strong><em class="status ${p.status==='available'?'open':'hot'}">${p.status==='available'?'제안 가능':'영입 경쟁'}</em></article>`).join(''):'<div class="loading">등록된 선수가 없습니다.</div>';
  document.getElementById('playerGrid').innerHTML=ps.map(p=>`<article class="profile-card"><div class="rating">${p.ability??'—'}</div><span class="position">${esc(p.position||'—')}</span><h3>${esc(p.name)}</h3><p>${esc(p.nationality||'—')} • ${esc(p.clubs?.name||'자유계약')}</p><div class="profile-meta"><span>어빌 <b>${p.ability??'—'}</b></span><span>포텐 <b>${p.potential??'—'}</b></span><span>가치 <b>${money(p.market_value)}</b></span></div></article>`).join('');
  document.getElementById('clubGrid').innerHTML=cs.map(c=>`<div class="club-card"><span>${esc((c.name||'FT').slice(0,2).toUpperCase())}</span><b>${esc(c.name)}</b><small>${esc(c.league||'FANTASY LEAGUE')}</small></div>`).join('');
...
document.getElementById('offerForm').addEventListener('submit',async e=>{e.preventDefault();const playerId=document.getElementById('offerPlayer').value,clubId=document.getElementById('offerClub').value,fee=Number(document.getElementById('offerFee').value);const player=players.find(p=>p.id===playerId);const msg=document.getElementById('offerMessage');if(!player||fee<0){msg.textContent='입력값을 확인하세요.';return}const {error}=await db.from('transfer_offers').insert({player_id:playerId,buying_club_id:clubId,selling_club_id:player.current_club_id,fee,status:'pending'});if(error){console.error(error);msg.textContent='제안 제출에 실패했습니다.';return}msg.textContent='제안이 FTMA 시장에 등록되었습니다.';document.getElementById('offerForm').reset();await loadData();setTimeout(()=>dialog.close(),700)});
...
document.getElementById('loginBtn').addEventListener('click',()=>alert('구단 로그인/권한 시스템은 다음 단계에서 Supabase Auth로 연결할 수 있습니다.'));
loadData();
