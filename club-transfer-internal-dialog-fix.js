(function(){
  ...
  const financeFor=(clubId,excludeId=null)=>{
    const club=(realClubs||[]).find(c=>c.id===clubId);
    const total=Number(clubId===realClubId?realClub?.transfer_budget:club?.transfer_budget||0);
  ...
  .ftma-id-kicker{color:#caa968;font-size:9px;letter-spacing:.18em;font-weight:900}.ftma-id-modal h3{margin:8px 0 10px;font-size:23px}.ftma-id-message{color:#aaa69e;font-size:11px;line-height:1.75;padding:12px 0}.ftma-id-label{display:block;color:#aaa69e;font-size:10px;margin-top:12px}.ftma-id-label input,.ftma-id-label textarea{display:block;width:100%;box-sizing:border-box;margin-top:6px;background:#0a0c0e;border:1px solid #3a3d42;color:#eee;padding:11px;outline:0}.ftma-id-label textarea{min-height:90px;resize:vertical}.ftma-id-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:20px}.ftma-id-btn{border:1px solid #45484d;background:#181a1d;color:#ddd;padding:11px 17px;font-size:10px;font-weight:800;cursor:pointer}.ftma-id-btn.primary{background:#caa968;border-color:#caa968;color:#111}`;document.head.appendChild(style);

  window.alert=message=>{let text=String(message);const selected=(realOffers||[]).find(o=>o.id===realSelected);if(selected&&selected.selling_club_id===realClubId&&text.includes('이적 자금이 부족합니다.'))text=`상대 구단의 이적 자금이 부족합니다.\n${text.replace(/^.*?가용 자금/, '가용 자금')}`;dialog(text,{title:'이적 자금 부족'});};
