...
const cacheHeaders={'Content-Type':'application/json; charset=utf-8','Cache-Control':'public, s-maxage=120, stale-while-revalidate=600','CDN-Cache-Control':'public, s-maxage=120, stale-while-revalidate=600'};
async function supabase(path){const r=await fetch(SUPABASE_URL+path,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`}});if(!r.ok)throw new Error(`Supabase HTTP ${r.status}`);return r.json()}
...
export default async function handler(req,res){try{const u=new URL(req.url,`https://${req.headers.host||'ftam-transfer.vercel.app'}`),resource=u.searchParams.get('resource')||'home',id=u.searchParams.get('id');let data;
if(resource==='home'){
 const [players,summary,clubs,news,transfers]=await Promise.all([
  supabase(`/rest/v1/players?select=${encodeURIComponent(playerFields)}&order=market_value.desc&limit=12`),
  supabase('/rest/v1/players?select=market_value,squad_type&limit=1000'),
 ...
 data={players,clubs,news,transfers,summary:{players:summary.length,clubs:clubs.length,u20:summary.filter(p=>p.squad_type==='u20').length,market_value:summary.reduce((n,p)=>n+Number(p.market_value||0),0)}};
 ...
 if(id){const rows=await supabase(`/rest/v1/news?select=id,title,summary,body,cover_image_url,category,published,published_at&id=eq.${encodeURIComponent(id)}&published=eq.true&limit=1`);data=rows[0]||null}
 else data=await supabase('/rest/v1/news?select=id,title,summary,cover_image_url,category,published,published_at&published=eq.true&order=published_at.desc');
}else return res.status(400).setHeader('Content-Type','application/json').json({error:'invalid_resource'});
return res.status(200).set(cacheHeaders).json(data)}catch(e){return res.status(502).set(cacheHeaders).json({error:e?.message||'public_data_unavailable'})}}
